{{-- مسكن — صفحة إضافة عقار جديد --}}
@extends('layouts.app')

@section('title', __('Add Property - Maskan'))

@section('content')
<div class="page-header">
    <h1>{{ __('Add New Property') }}</h1>
    <a href="{{ route('properties.index') }}" class="btn btn-outline-gold">
        <i class="fas fa-arrow-right ms-1"></i> {{ __('Back to List') }}
    </a>
</div>
...
<div class="maskan-card">
    <div class="card-body">
        <form action="{{ route('properties.store') }}" method="POST" class="form-maskan" enctype="multipart/form-data">
            ...
            <div class="row">
                <div class="col-md-8 mb-3">
                    ...
                    <input type="text" name="title" class="form-control @error('title') is-invalid @enderror"
                           placeholder="{{ __('Enter property title') }}" value="{{ old('title') }}" required>
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <select name="type" class="form-select @error('type') is-invalid @enderror" required>
                        ...
                        <option value="resort" {{ old('type') == 'resort' ? 'selected' : '' }}>{{ __('Resort') }}</option>
                        <option value="rest_house" {{ old('type') == 'rest_house' ? 'selected' : '' }}>{{ __('Rest House') }}</option>
                        <option value="villa" {{ old('type') == 'villa' ? 'selected' : '' }}>{{ __('Villa') }}</option>
                        <option value="house" {{ old('type') == 'house' ? 'selected' : '' }}>{{ __('House') }}</option>
                        <option value="building" {{ old('type') == 'building' ? 'selected' : '' }}>{{ __('Building') }}</option>
            ...
            <div class="mb-3">
                <label class="form-label">{{ __('Property Description') }} <span class="text-danger">*</span></label>
                <textarea name="description" class="form-control @error('description') is-invalid @enderror"
                          rows="4" placeholder="{{ __('Describe the property in detail') }}" required>{{ old('description') }}</textarea>
            ...
            <div class="mb-3">
                ...
                <div class="d-flex align-items-center gap-3">
                    ...
                    <span id="locationStatus" class="text-muted small">{{ __('Not set') }}</span>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-md-4 mb-3">
                    <label class="form-label">{{ __('Price per Night (LYD)') }} <span class="text-danger">*</span></label>
                    <input type="number" step="0.01" min="0" name="price_per_night"
                           class="form-control @error('price_per_night') is-invalid @enderror"
                           placeholder="150.00" value="{{ old('price_per_night') }}" required>
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <input type="number" min="0" name="area_sqm"
                           ...
                           placeholder="120" value="{{ old('area_sqm') }}">
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <input type="number" step="0.01" min="0" name="price_per_month"
                           ...
                           placeholder="1000" value="{{ old('price_per_month') }}">
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <input type="number" step="0.01" min="0" name="deposit"
                           ...
                           placeholder="500" value="{{ old('deposit') }}">
            ...
            <div class="row">
                <div class="col-md-4 mb-3">
                    ...
                    <input type="number" min="0" name="rooms_count"
                           ...
                           placeholder="2" value="{{ old('rooms_count') }}" required>
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <input type="number" min="0" name="bathrooms_count"
                           ...
                           placeholder="1" value="{{ old('bathrooms_count') }}" required>
            ...
            <div class="mb-4">
                ...
                <div class="d-flex flex-wrap gap-4">
                    <div class="form-check">
                        <input type="checkbox" name="has_pool" value="1" class="form-check-input" id="has_pool" {{ old('has_pool') ? 'checked' : '' }}>
                        <label class="form-check-label" for="has_pool">{{ __('Pool') }}</label>
                    </div>
                    <div class="form-check">
                        <input type="checkbox" name="has_parking" value="1" class="form-check-input" id="has_parking" {{ old('has_parking') ? 'checked' : '' }}>
                        <label class="form-check-label" for="has_parking">{{ __('Parking') }}</label>
                    </div>
                    <div class="form-check">
                        <input type="checkbox" name="has_ac" value="1" class="form-check-input" id="has_ac" {{ old('has_ac') ? 'checked' : '' }}>
                        <label class="form-check-label" for="has_ac">{{ __('Air Conditioning') }}</label>
                    </div>
                    <div class="form-check">
                        <input type="checkbox" name="has_furniture" value="1" class="form-check-input" id="has_furniture" {{ old('has_furniture') ? 'checked' : '' }}>
            ...
            <div class="mb-4">
                ...
                <div class="image-uploader" onclick="this.querySelector('input').click()">
                    ...
                    <div class="upload-text">{{ __('Click to upload images') }}<br><small>{{ __('Upload hint') }}</small></div>
            ...
            <div class="d-flex gap-2">
                <button type="submit" class="btn btn-maskan px-4">
                    <i class="fas fa-save ms-1"></i> {{ __('Save Property') }}
                </button>
                <a href="{{ route('properties.index') }}" class="btn btn-outline-secondary px-4">{{ __('Cancel') }}</a>
            </div>
        </form>
    </div>
</div>
